...
class Board(object):
    ...
    def is_valid_path(self, path):
        ...
        def _is_valid_path(path):
            ...
            neighbors = [n.territory_id for n in neighbors]
            if path[1] in neighbors:
                return _is_valid_path(path[1:])
            else:
                return False
    ...
    def is_valid_attack_path(self, path):
        ...
        for territory in path[1:]:
            if self.owner(path[0]) == self.owner(territory):
                return False
        return True
    ...
    def cost_of_attack_path(self, path):
        ...
        enemy_armies = 0
        for territory in path[1:]:
            enemy_armies += self.armies(territory)
    ...
    def shortest_path(self, source, target):
        ...
        queue = deque()
        queue.append(source)
        visited_territories = set([source])
        ...
        while len(queue) != 0:
            ...
            for territory in risk.definitions.territory_neighbors[current_territory]:
                if territory not in visited_territories:
                    ...
                    new_stack = copy.copy(dictionary[current_territory])
                    new_stack.append(territory)
                    if territory == target:
                        return new_stack
                    dictionary[territory] = new_stack
    ...
    def can_fortify(self, source, target):
        ...
        while len(queue) != 0:
            current_territory = queue.popleft()
            friendly = self.friendly_neighbors(current_territory)
            friendly = [f.territory_id for f in friendly]
            for territory in friendly:
                if territory not in visited_territories:
                    visited_territories.add(territory)
                    new_stack = copy.copy(dictionary[current_territory])
                    new_stack.append(territory)
    ...
    def cheapest_attack_path(self, source, target):
        ...
        while len(queue) != 0:
            current_territory = queue.popitem()
            for territory in risk.definitions.territory_neighbors[current_territory[0]]:
                if territory not in visited_territories and self.owner(source) != self.owner(territory):
                    visited_territories.add(territory)
                    new_stack = copy.copy(dictionary[current_territory[0]])
                    new_stack.append(territory)
                    if territory == target:
                        return new_stack
                    priority = current_territory[1] + self.armies(territory)
                    dictionary[territory] = new_stack
                    queue[territory] = priority
